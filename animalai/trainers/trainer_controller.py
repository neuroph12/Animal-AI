...
class TrainerController(object):
    ...
    def _reset_env(self, env):
        ...
        if self.meta_curriculum is not None:
            ...
            if self.update_config:
                print("B") ## ここは関係がない。なぜなら, play=true,falseでも呼ばれたから。
    ...
    def start_learning(self, env, trainer_config):
        ...
        curr_info = self._reset_env(env) # play=falseの場合はtest5に遷移するのに, play=trueの場合はここで止まる。なぜだ。
    ...
    def take_step(self, env, curr_info):
        ...
        take_action_vector, take_action_memories, take_action_text, take_action_value, take_action_outputs = {}, {}, {}, {}, {}
        for brain_name, trainer in self.trainers.items():
            (take_action_vector[brain_name], take_action_memories[brain_name], take_action_text[brain_name], take_action_value[brain_name], take_action_outputs[brain_name]) = trainer.take_action(curr_info)

        print("take_action_vector = {}".format(take_action_vector))
        print("take_action_text = {}".format(take_action_text))
        print("take_action_value = {}".format(take_action_value))
